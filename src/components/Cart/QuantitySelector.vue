...
const updateQuantity = (event: Event) => {
	...
	if (!isNaN(value)) {
		const validValue = Math.max(props.min ?? 1, Math.min(props.max ?? Infinity, value))
...
<template>
	<div class="flex h-10 items-center overflow-hidden rounded-full border border-gray-200 bg-white">
		...
		>
			...
			<template v-else>
				<svg width="14" height="2" viewBox="0 0 14 2" fill="none" xmlns="http://www.w3.org/2000/svg">
					<path d="M1 1H13" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
				</svg>
			</template>
		</button>
		...
		<input
			type="text"
			class="h-full w-10 border-none bg-white text-center text-sm focus:outline-none focus:ring-0"
			:value="quantity"
		...
		>
			<svg width="14" height="14" viewBox="0 0 14 14" fill="none" xmlns="http://www.w3.org/2000/svg">
				<path d="M7 1V13M1 7H13" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
			</svg>
		</button>
	</div>
</template>
